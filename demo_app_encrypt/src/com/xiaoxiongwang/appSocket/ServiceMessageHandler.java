package com.xiaoxiongwang.appSocket;
...
import com.xiaoxiongwang.domain.UserLoginReq;
import com.xiaoxiongwang.domain.UserLoginRes;
import com.xiaoxiongwang.service.LoginService;
import com.xiaoxiongwang.util.GoogleCacheUtil;
...
public class ServiceMessageHandler extends MinaServerHanlder {
	...
	public void messageHandler(IoSession session, Object message) {
		...
		if (Mid.userLoginReq == mes.getInt("mid")) {
			...
			int resLoginCount = loginService.socketOnLine(ulq.getUserId());
			//判断用户是否登陆成功!
			//如果为0表示 没有此用户
			//关闭session
			if(1 != resLoginCount) {
