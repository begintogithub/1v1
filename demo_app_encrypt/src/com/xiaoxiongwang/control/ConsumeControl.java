...
import com.xiaoxiongwang.util.BaseUtil;
import com.xiaoxiongwang.util.MessageUtil;
import com.xiaoxiongwang.util.RSACoderUtil;
...
public class ConsumeControl {
	 ...
	 * @param userId    用户编号
	 * @param setMealId 套餐编号
	 * @param payType   支付渠道 0.支付宝 1.微信 2.码支付_支付宝 3.码支付_微信
	 * @param response
	 */
	@RequestMapping(value = "goldStoreValue", method = RequestMethod.POST)
	@ResponseBody
	public MessageUtil goldStoreValue(HttpServletRequest req, HttpServletResponse response) {
		// 解密参数
		JSONObject param = RSACoderUtil.privateDecrypt(req);
		// 验证传递的参数
		if (!BaseUtil.params(param.getInt("userId"), param.getInt("setMealId")) && param.getInt("payType") < 0
