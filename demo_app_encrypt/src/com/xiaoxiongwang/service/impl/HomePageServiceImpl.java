...
import com.xiaoxiongwang.domain.LoginInfo;
import com.xiaoxiongwang.domain.UserIoSession;
import com.xiaoxiongwang.domain.WalletDetail;
import com.xiaoxiongwang.service.HomePageService;
...
public class HomePageServiceImpl extends ICommServiceImpl implements HomePageService {
	...
	public MessageUtil getHomePageList(int userId, int page, Integer queryType) {
		try {
			...
			homeSql.append(
					"SELECT u.t_id,u.t_cover_img,a.t_state,u.t_handImg,u.t_nickName,u.t_age,u.t_city,u.t_vocation,u.t_autograph,s.t_video_gold,AVG(IFNULL(e.t_score,5)) AS t_score,'1' AS so ");
			homeSql.append(
					"FROM (SELECT * FROM t_user WHERE t_onLine = 0 AND t_role = 1 AND t_disable = 0  AND t_cover_img is not null AND t_sex = 0 ) u LEFT JOIN t_anchor a ON a.t_user_id = u.t_id ");
			...
			homeSql.append("WHERE u.t_id IN (");
			homeSql.append(StringUtils.isNotBlank(inStr) ? inStr.substring(0, inStr.length() - 1) : 0).append(") ");
//			homeSql.append("AND a.t_state !=2 ");
			homeSql.append("GROUP BY u.t_id ");
			homeSql.append("UNION ");
			homeSql.append(
					"SELECT u.t_id,u.t_cover_img,a.t_state,u.t_handImg,u.t_nickName,u.t_age,u.t_city,u.t_vocation,u.t_autograph,s.t_video_gold,AVG(IFNULL(e.t_score,5)) AS t_score,'2' AS so ");
			homeSql.append(
					"FROM (SELECT * FROM t_user WHERE t_onLine = 0 AND t_role = 1 AND t_disable = 0  AND t_cover_img is not null AND t_sex = 0 ) u LEFT JOIN t_anchor a ON a.t_user_id = u.t_id ");
			homeSql.append("LEFT JOIN t_user_evaluation e ON e.t_user_id = u.t_id ");
			homeSql.append("LEFT JOIN t_anchor_setup s ON  s.t_user_id = u.t_id ");
			homeSql.append("WHERE u.t_id NOT IN (");
			homeSql.append(StringUtils.isNotBlank(inStr) ? inStr.substring(0, inStr.length() - 1) : 0);
			homeSql.append(")  ");
//			homeSql.append(" AND a.t_state !=2 ");
			homeSql.append("GROUP BY u.t_id ");
			// 统计总记录数
			...
			// 获取数据
			List<Map<String, Object>> dataList = this.getQuerySqlList("SELECT aa.* FROM (" + homeSql
					+ ") aa  LEFT JOIN t_certification c ON c.t_user_id = aa.t_id ORDER BY aa.so ASC,c.t_certification_type DESC,aa.t_state ASC,aa.t_score DESC LIMIT ?,10",
					(page - 1) * 10);
	 ...
	 * 
	 * @see com.xiaoxiongwang.service.app.HomePageService#getAnchorPlayPage(int)
	 */
	@Override
	public MessageUtil getAnchorPlayPage(int consumeUserId, int coverConsumeUserId, Integer albumId,
		...
		try {
			...
			List<Map<String, Object>> isWeiXin = this.getFinalDao().getIEntitySQLDAO().findBySQLTOMap(sql,
			...
			List<Map<String, Object>> laudList = this.getFinalDao().getIEntitySQLDAO().findBySQLTOMap(sql,
					consumeUserId, coverConsumeUserId);
			...
			List<Map<String, Object>> followList = this.getFinalDao().getIEntitySQLDAO().findBySQLTOMap(sql,
					consumeUserId, coverConsumeUserId);
	 ...
	 * 
	 * @see com.xiaoxiongwang.service.app.HomePageService#getBannerList()
	 */
	@Override
	 ...
	 * 获取同城数据(non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.HomePageService#getCityWideList(int, int)
	 */
	@Override
	 ...
	 * 获取搜索列表(non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.HomePageService#getSearchList(int, java.lang.String)
	 */
	@Override
	 ...
	 * 获取消费排行榜(non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.HomePageService#getConsumeList(int, int)
	 */
	@Override
	 ...
	 * 获取豪礼排行榜(non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.HomePageService#getCourtesyList(int, int)
	 */
	@Override
