...
import com.xiaoxiongwang.service.NearbyService;
...
public class NearbyServiceImpl extends ICommServiceImpl implements
	 ...
	 * 上传用户坐标 (non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.app.NearbyService#uploadCoordinate(int,
	 ...
	 * 获取附近的用户数据
	 * (non-Javadoc)
	 * @see com.xiaoxiongwang.service.app.NearbyService#getNearbyList(java.lang.Double, java.lang.Double)
	 */
	@Override
	...
	public MessageUtil getAnthorDistanceList(int userId, int page,double lat,double lng) {
		try {
			...
			body.append(" (");
			body.append(" acos(");
			body.append("sin((t_lat * 3.1415) / 180) * sin((? * 3.1415) / 180) + cos((t_lat * 3.1415) / 180) * cos((? * 3.1415) / 180) * cos(");
			body.append("(t_lng * 3.1415) / 180 - (? * 3.1415) / 180");
			body.append(")) * 6370.996 ");
			body.append(") AS distance ");
			body.append("FROM t_coordinate c LEFT JOIN t_user u ON t_user_id = u.t_id  LEFT JOIN t_anchor a ON a.t_user_id = u.t_id ");
			body.append("WHERE ");
			body.append(" u.t_sex <>  ").append(map.get("t_sex"));
			body.append(" AND u.t_role <> ").append(map.get("t_role"));
			
			Map<String, Object> totalMap = this.getMap("SELECT COUNT(*) AS total FROM ("+body+") aa ", lat,lat,lng);
			
			body.append(" ORDER BY u.t_onLine ASC,");
			body.append("distance ASC ");
			
			
			List<Map<String,Object>> qlist = this.getQuerySqlList(body+ " LIMIT ?,10", lat,lat,lng,(page-1)*10);
