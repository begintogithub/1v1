...
import com.xiaoxiongwang.domain.OnLineRes;
...
import com.xiaoxiongwang.evnet.PushLinkUser;
...
import com.xiaoxiongwang.util.StopThread;
import com.xiaoxiongwang.util.SystemConfig;
import com.xiaoxiongwang.util.WebRTCSigApi;
...
public class VideoChatServiceImpl extends ICommServiceImpl implements VideoChatService {
	 ...
	 * 
	 * @see com.xiaoxiongwang.service.VideoChatService#getVideoChatPriavteMapKey(int, int)
	 */
	@Override
...
//			}
	 ...
	 * 用户挂断链接(non-Javadoc)
	 * 
	 * @see com.xiaoxiongwang.service.VideoChatService#breakLink(int, int)
	 */
	@Override
	public MessageUtil breakLink(int roomId, int type) {
		try {
			...
			if((null != room && 0 != room.getLaunchUserId() && 0 != room.getCoverLinkUserId()) || (null != VideoTiming.getRoom(roomId))){
				...
				if (null != ioSession) {
					ioSession.write(JSONObject.fromObject(new com.xiaoxiongwang.domain.Mid(Mid.brokenLineRes)).toString());
				}

				// 给被链接人推送挂断信息
				IoSession coverIoSession = UserIoSession.getInstance().getMapIoSession(room.getCoverLinkUserId());
				if (null != coverIoSession) {
					coverIoSession.write(JSONObject.fromObject(new com.xiaoxiongwang.domain.Mid(Mid.brokenLineRes)).toString());
				}
