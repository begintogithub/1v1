...
public class PushUtil {
    ...
    protected static final String MASTER_SECRET = SystemConfig.getValue("master_secret");
	...
	public static void sendPush(int userId,String content) {
		ClientConfig clientConfig = ClientConfig.getInstance();
        final JPushClient jpushClient = new JPushClient(MASTER_SECRET, APP_KEY, null, clientConfig);
      
        PushPayload payload = buildPushObject_all_alias_alert(userId +"",content);
        try {
	...
	public static void main(String[] args) throws Exception {
		...
		sendTestPush(4, "周阳",1,5,0);
		
//		sendPush(4, "周阳");
	}
	
	
	public static void sendPush(int userId, String msg_content, String extrasparam) {
		ClientConfig clientConfig = ClientConfig.getInstance();
        final JPushClient jpushClient = new JPushClient(MASTER_SECRET, APP_KEY, null, clientConfig);
        PushPayload payload = buildPushObject_all_registrationId_alertWithTitle(userId+"", msg_content, extrasparam);
        try {
            PushResult result = jpushClient.sendPush(payload);
            LOG.info("Got result - " + result);
            System.out.println(result);
	...
	public static void sendTestPush(int userId, String msg_content,int roomId,int connectUserId,int satisfy) {
		ClientConfig clientConfig = ClientConfig.getInstance();
        final JPushClient jpushClient = new JPushClient(MASTER_SECRET, APP_KEY, null, clientConfig);
      
        try {
        	
        	final PushPayload payload = buildPushObject_android_and_ios(userId,msg_content,roomId,connectUserId,satisfy);
     ...
     * @return
     */
    private static PushPayload buildPushObject_all_registrationId_alertWithTitle(String registrationId, String msg_content, String extrasparam) {
 
        return PushPayload.newBuilder()
                //指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
                .setPlatform(Platform.all())
                //指定推送的接收对象，all代表所有人，也可以指定已经设置成功的tag或alias或该应应用客户端调用接口获取到的registration id Audience.alias(id)
                ...
                .setMessage(Message.newBuilder()
                        ...
                        .addExtra("noticeType", extrasparam)
                        .build())
...
//    }
	...
	//指定推送
    public static PushPayload buildPushObject_all_alias_alert(String id,String content) {
        return PushPayload.newBuilder()
                .setPlatform(Platform.all())
    ...
    public static PushPayload buildPushObject_android_and_ios(int deviceId,String msg_content,int roomId,int connectUserId,int satisfy ) {
        ...
        return PushPayload.newBuilder()
                ...
                .setNotification(Notification.newBuilder()
                		...
                		.addPlatformNotification(IosNotification.newBuilder()
                				...
                				.setCategory("NotificationServiceZY")
                				.setMutableContent(true)
                				.setSound("call2.wav")
                				.addExtra("roomId", roomId)
                				.addExtra("connectUserId", connectUserId)
                				.addExtra("satisfy", satisfy)
                				.build())
