...
public class WebRTCSigApi {
	...
	private volatile static WebRTCSigApi webRTCSigApi;

	private WebRTCSigApi() {
	}

	public static WebRTCSigApi getInstance(){
		if (webRTCSigApi == null) {
			synchronized (WebRTCSigApi.class) {
				if (webRTCSigApi == null) {
					webRTCSigApi = new WebRTCSigApi();
					webRTCSigApi.setSdkAppid(Integer.parseInt(SystemConfig.getValue("SDKAppid")));
					webRTCSigApi.setPublicKey(SystemConfig.getValue("tx_public_key"));
					webRTCSigApi.setPrivateKey(SystemConfig.getValue("tx_private_key"));
				}
			}
		}
		return webRTCSigApi;
	...
	public String genUserSig(String userid, int expire) {
		...
		String jsonString = "{" + "\"TLS.account_type\":\"" + 0 + "\","
				...
				+ "\"TLS.appid_at_3rd\":\"" + 0 + "\","
				+ "\"TLS.sdk_appid\":\"" + this.mSdkAppid + "\","
				+ "\"TLS.expire_after\":\"" + expire + "\"," + "\"TLS.sig\":\""
				+ sig + "\"," + "\"TLS.time\":\"" + time + "\","
		...
		compresser.end();
		String userSig = new String(base64UrlEncode(Arrays.copyOfRange(
				compressBytes, 0, compressBytesLength)));
	...
	public String genPrivateMapKey(String userid, int roomid, int expire) {
		...
		// 视频校验位需要用到的字段
		/*
		 * cVer unsigned char/1 版本号，填0 wAccountLen unsigned short /2 第三方自己的帐号长度
		 * buffAccount wAccountLen 第三方自己的帐号字符 dwSdkAppid unsigned int/4 sdkappid
		 * dwRoomId unsigned int/4 群组号码 dwExpTime unsigned int/4 过期时间 （当前时间 +
		 * 有效期（单位：秒，建议300秒）） dwPrivilegeMap unsigned int/4 权限位 dwAccountType
		 * unsigned int/4 第三方帐号类型
		 */
		...
		String jsonString = "{" + "\"TLS.appid_at_3rd\":\"" + 0 + "\","
				+ "\"TLS.account_type\":\"" + 0 + "\","
				+ "\"TLS.identifier\":\"" + userid + "\","
				+ "\"TLS.sdk_appid\":\"" + this.mSdkAppid + "\","
				+ "\"TLS.expire_after\":\"" + expire + "\"," + "\"TLS.sig\":\""
		...
		int compressBytesLength = compresser.deflate(compressBytes);
		compresser.end();
		String privateMapKey = new String(base64UrlEncode(Arrays.copyOfRange(
				compressBytes, 0, compressBytesLength)));
